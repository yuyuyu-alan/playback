...
class RoomController {
  ...
  listenPubMsg() {
    ...
    rtcEngine.on("onPubMsg", (event) => {
      ...
      // 		data.layout = currLayoutType === 'singleToRight' ? ConstantController.LAYOUT_TYPE.SINGLE_TO_RIGHT : ConstantController.LAYOUT_TYPE.SINGLE_TO_BOTTOM;
      // 	}
      ...
      // 		this.sendSignalingMessage(data.whiteboardInfo.name, data.whiteboardInfo.data, data.whiteboardInfo.type);
      // 	}

      // 	if (Number(data.layout) === ConstantController.LAYOUT_TYPE.DEFAULT) { // 切回默认布局
      // 		store.commit('setData', {currFocusIndex: -1});
      // 		LayoutController.watchWindowResize();
      // 	}
      // } else if (id === 'AllForbidTalk') {
      // 	const {Teacher = {}} = store.state;
      // 	const {teacher} = Teacher;
      // 	//更改聊天状态
      // 	store.commit('setData', {chatStatus: ConstantController.CHAT_STATUS.DISABLE});
      // 	this.sendChatMessage('老师开启了全体禁言', {
      // 		nickname: '',
      // 		type: ConstantController.CHAT_TYPE.SYSTEM
      // 	}, teacher.uid);
      // } else if (id === 'Notice_BigRoom_Usernum') {
      // 	//记录直播房间人数
